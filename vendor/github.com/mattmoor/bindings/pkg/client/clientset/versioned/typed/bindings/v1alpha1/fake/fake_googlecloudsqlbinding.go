/*
Copyright 2019 The Knative Authors

Licensed under the Apache License, Version 2.0 (the "License");
you may not use this file except in compliance with the License.
You may obtain a copy of the License at

    http://www.apache.org/licenses/LICENSE-2.0

Unless required by applicable law or agreed to in writing, software
distributed under the License is distributed on an "AS IS" BASIS,
WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
See the License for the specific language governing permissions and
limitations under the License.
*/

// Code generated by client-gen. DO NOT EDIT.

package fake

import (
	v1alpha1 "github.com/mattmoor/bindings/pkg/apis/bindings/v1alpha1"
	v1 "k8s.io/apimachinery/pkg/apis/meta/v1"
	labels "k8s.io/apimachinery/pkg/labels"
	schema "k8s.io/apimachinery/pkg/runtime/schema"
	types "k8s.io/apimachinery/pkg/types"
	watch "k8s.io/apimachinery/pkg/watch"
	testing "k8s.io/client-go/testing"
)

// FakeGoogleCloudSQLBindings implements GoogleCloudSQLBindingInterface
type FakeGoogleCloudSQLBindings struct {
	Fake *FakeBindingsV1alpha1
	ns   string
}

var googlecloudsqlbindingsResource = schema.GroupVersionResource{Group: "bindings.mattmoor.dev", Version: "v1alpha1", Resource: "googlecloudsqlbindings"}

var googlecloudsqlbindingsKind = schema.GroupVersionKind{Group: "bindings.mattmoor.dev", Version: "v1alpha1", Kind: "GoogleCloudSQLBinding"}

// Get takes name of the googleCloudSQLBinding, and returns the corresponding googleCloudSQLBinding object, and an error if there is any.
func (c *FakeGoogleCloudSQLBindings) Get(name string, options v1.GetOptions) (result *v1alpha1.GoogleCloudSQLBinding, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewGetAction(googlecloudsqlbindingsResource, c.ns, name), &v1alpha1.GoogleCloudSQLBinding{})

	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.GoogleCloudSQLBinding), err
}

// List takes label and field selectors, and returns the list of GoogleCloudSQLBindings that match those selectors.
func (c *FakeGoogleCloudSQLBindings) List(opts v1.ListOptions) (result *v1alpha1.GoogleCloudSQLBindingList, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewListAction(googlecloudsqlbindingsResource, googlecloudsqlbindingsKind, c.ns, opts), &v1alpha1.GoogleCloudSQLBindingList{})

	if obj == nil {
		return nil, err
	}

	label, _, _ := testing.ExtractFromListOptions(opts)
	if label == nil {
		label = labels.Everything()
	}
	list := &v1alpha1.GoogleCloudSQLBindingList{ListMeta: obj.(*v1alpha1.GoogleCloudSQLBindingList).ListMeta}
	for _, item := range obj.(*v1alpha1.GoogleCloudSQLBindingList).Items {
		if label.Matches(labels.Set(item.Labels)) {
			list.Items = append(list.Items, item)
		}
	}
	return list, err
}

// Watch returns a watch.Interface that watches the requested googleCloudSQLBindings.
func (c *FakeGoogleCloudSQLBindings) Watch(opts v1.ListOptions) (watch.Interface, error) {
	return c.Fake.
		InvokesWatch(testing.NewWatchAction(googlecloudsqlbindingsResource, c.ns, opts))

}

// Create takes the representation of a googleCloudSQLBinding and creates it.  Returns the server's representation of the googleCloudSQLBinding, and an error, if there is any.
func (c *FakeGoogleCloudSQLBindings) Create(googleCloudSQLBinding *v1alpha1.GoogleCloudSQLBinding) (result *v1alpha1.GoogleCloudSQLBinding, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewCreateAction(googlecloudsqlbindingsResource, c.ns, googleCloudSQLBinding), &v1alpha1.GoogleCloudSQLBinding{})

	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.GoogleCloudSQLBinding), err
}

// Update takes the representation of a googleCloudSQLBinding and updates it. Returns the server's representation of the googleCloudSQLBinding, and an error, if there is any.
func (c *FakeGoogleCloudSQLBindings) Update(googleCloudSQLBinding *v1alpha1.GoogleCloudSQLBinding) (result *v1alpha1.GoogleCloudSQLBinding, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewUpdateAction(googlecloudsqlbindingsResource, c.ns, googleCloudSQLBinding), &v1alpha1.GoogleCloudSQLBinding{})

	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.GoogleCloudSQLBinding), err
}

// UpdateStatus was generated because the type contains a Status member.
// Add a +genclient:noStatus comment above the type to avoid generating UpdateStatus().
func (c *FakeGoogleCloudSQLBindings) UpdateStatus(googleCloudSQLBinding *v1alpha1.GoogleCloudSQLBinding) (*v1alpha1.GoogleCloudSQLBinding, error) {
	obj, err := c.Fake.
		Invokes(testing.NewUpdateSubresourceAction(googlecloudsqlbindingsResource, "status", c.ns, googleCloudSQLBinding), &v1alpha1.GoogleCloudSQLBinding{})

	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.GoogleCloudSQLBinding), err
}

// Delete takes name of the googleCloudSQLBinding and deletes it. Returns an error if one occurs.
func (c *FakeGoogleCloudSQLBindings) Delete(name string, options *v1.DeleteOptions) error {
	_, err := c.Fake.
		Invokes(testing.NewDeleteAction(googlecloudsqlbindingsResource, c.ns, name), &v1alpha1.GoogleCloudSQLBinding{})

	return err
}

// DeleteCollection deletes a collection of objects.
func (c *FakeGoogleCloudSQLBindings) DeleteCollection(options *v1.DeleteOptions, listOptions v1.ListOptions) error {
	action := testing.NewDeleteCollectionAction(googlecloudsqlbindingsResource, c.ns, listOptions)

	_, err := c.Fake.Invokes(action, &v1alpha1.GoogleCloudSQLBindingList{})
	return err
}

// Patch applies the patch and returns the patched googleCloudSQLBinding.
func (c *FakeGoogleCloudSQLBindings) Patch(name string, pt types.PatchType, data []byte, subresources ...string) (result *v1alpha1.GoogleCloudSQLBinding, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewPatchSubresourceAction(googlecloudsqlbindingsResource, c.ns, name, pt, data, subresources...), &v1alpha1.GoogleCloudSQLBinding{})

	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.GoogleCloudSQLBinding), err
}
